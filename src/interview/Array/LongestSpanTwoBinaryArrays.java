 ...
 *        arr2[] = {1, 1, 1, 1, 1, 0, 1};
 * Output: 6 The longest span with same sum is from index 1 to 6.
 *
 * Input: arr1[] = {0, 0, 0};
 *        arr2[] = {1, 1, 1};
 * Output: 0
...
public class LongestSpanTwoBinaryArrays {
  ...
  static int longestCommonSum(int n) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      else if (diff[diffIndex] == Integer.MIN_VALUE) {
        diff[diffIndex] = i;
      }
      ...
      // Current diff is already seen
        ...
        if (len > maxLen) {
          System.out.println("i=" + i + ":index=" + diff[diffIndex]);
          maxLen = len;
        }
      }
    }
  ...
  static int longestCommonSum(int arr1[], int arr2[], int n) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (integerMap.containsKey(currDiff)) {
        max_len = Math.max(max_len, i - integerMap.get(currDiff));
      } else {// Else put this sum in hash table
        integerMap.put(currDiff, i);
      }
    }
