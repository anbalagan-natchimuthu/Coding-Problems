 ...
 *
 * This is a Java Program to implement Bloom Filter. A Bloom filter is a space-efficient probabilistic data structure
 * that is used to test whether an element is a member of a set. False positive matches are possible, but false
 * negatives are not; i.e. a query returns either “inside set (may be wrong)” or “definitely not in set”. Elements
 * can be added to the set, but not removed (though this can be addressed with a “counting” filter). The more
