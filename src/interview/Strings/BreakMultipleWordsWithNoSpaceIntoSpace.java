...
public class BreakMultipleWordsWithNoSpaceIntoSpace {
  ...
  /**
   * VERSION: 2 Using BackTracking.. refer SubSets_Permutations.java file for more similar problem.
   */
  public List<String> wordBreak(String s, List<String> wordDict) {
    List<List<String>> result = new ArrayList<>();
    helper(s, wordDict, 0, result, new ArrayList<>());

    List<String> finalStr = new ArrayList<>();
    for (List<String> list : result) {
      finalStr.add(String.join(" ", list));
    }
    return finalStr;
  }

  public void helper(String s, List<String> wordDict, int start, List<List<String>> result,
    ...
    for (int i = start; i < s.length(); i++) {
      ...
      if (wordDict.contains(subString)) {
        ...
        helper(s, wordDict, i + 1, result, tempList);
  ...
  private boolean wordBreakTopDownOneSolutionUtil(String s, Set<String> dict, int start, int max,
