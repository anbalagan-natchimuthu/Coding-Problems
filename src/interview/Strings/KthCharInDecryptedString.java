 ...
 * For example, if encrypted string is “ab2cd2” and k=4 , so output will be ‘b’ because decrypted string is “ababcdcd”
 * and 4th character is ‘b’.

 Note: Frequency of encrypted substring can be of more than one digit. For example, in “ab12c3”, ab is repeated 12
 times.
...
public class KthCharInDecryptedString {
    ...
    static char encodedChar(String str, int k) {
        ...
        StringBuilder temp = new StringBuilder();  // Current substring
        int freq = 0; // Count of current substring

        for (int i = 0; i < str.length(); ) {
            temp = new StringBuilder(); // Current substring
